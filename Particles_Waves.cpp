...
struct Particle {
	...
	static VkVertexInputBindingDescription getBindingDescription() {
		...
		bindingDescription.stride = sizeof(Particle); // Should be 48
...
private:
	...
	void createGraphicsPipeline() {
		auto vertShaderCode = readFile("shaders_compute/vert.spv");
		auto fragShaderCode = readFile("shaders_compute/frag.spv");
		...
		// Create descriptor set layout for graphics pipeline
		...
		// Add to pipeline create info for depth
	...
	void createComputePipeline() {
		auto computeShaderCode = readFile("shaders_compute/comp_gerstner.spv"); //gerstner wave particle simulation
		//auto computeShaderCode = readFile("shaders_compute/comp.spv"); //random simulation
	...
	void createDescriptorPool() {
		...
		poolSizes[0].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT) * 2; // For both graphics and compute
		...
		poolInfo.maxSets = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT) * 2; // For both graphics and compute
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
			pipelineLayout, 0, 1, &graphicsDescriptorSets[currentFrame], 0, nullptr);
	...
	static void scrollCallback(GLFWwindow* window, double xoffset, double yoffset) {
		auto app = reinterpret_cast<ComputeShaderApplication*>(glfwGetWindowUserPointer(window));
		app->cameraSpeed = glm::clamp(app->cameraSpeed + (float)yoffset * 0.01f, 0.001f, 0.1f);
	}
